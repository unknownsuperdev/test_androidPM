...
class GetSearchedFinishBooksUseCaseImpl(
    ...
    override suspend fun execute(word: String): Flow<PagingData<BooksWithReadProgressBookData>> {
        return searchBookRepository.getSearchFinishedBooks(word).flow.map { pagingData ->
            pagingData.map {
                BooksWithReadProgressBookData.fromFinish(
                    it,
