package ru.tripster.guide.ui.calendar.model
...
import ru.tripster.guide.ui.calendar.utils.next
...
internal data class MonthConfig(
    ...
    internal val removePastDays: Boolean? = null,
) {
    internal var months: List<CalendarMonth> = run {
    ...
    internal companion object {
        ...
        internal fun generateWeekDays(
            ...
            val dateYear = date?.substring(0..3)?.toInt()
            val dateMonth = date?.substring(5..6)?.toInt()
            val dateDay = date?.substring(8..9)?.toInt()
            var isRemovable = false
            val thisMonthDays = (1..yearMonth.lengthOfMonth()).map {
                CalendarDay(LocalDate.of(year, month, it), DayOwner.THIS_MONTH)
            }
            var weekDaysGroup = if (generateInDates) {
            ...
            weekDaysGroup.forEach { list ->
                list.forEach { calendarDay ->
                    if (calendarDay.date == dateYear?.let { dateYear ->
                            dateMonth?.let { dateMonth ->
                                dateDay?.let { dateDay ->
            ...
            fun dropLineCount(): Int {
                val currentDay = currentDay()
                var dropLineCount = 0
                currentDay.let {
                    weekDaysGroup.forEachIndexed { index, item ->
                        item.forEach { day ->
                            if (day.date.toString().contains(it)) {
                                dropLineCount = index
                            }
                        }
                    }
                    return dropLineCount
                }
            }

            return weekDaysGroup.drop(dropLineCount())
        }
    }
}
...
private fun currentDay(): String =
    SimpleDateFormat("yyyy-MM-dd", Locale("ru")).format(Calendar.getInstance().time).toString()