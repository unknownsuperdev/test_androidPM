...
class EventsDatePagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ResultModel> {
        ...
        return try {
            val response =
                service.getEventsDateGte(status = status, sorting, startDate, page = page)
    ...
    class Factory {
        fun create(
            service: EventsApiService, status: Int,
            sorting: String,
            startDate: String
        ) =
            EventsDatePagingSource(service, status, sorting, startDate)
    }
}