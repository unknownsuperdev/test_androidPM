...
class EventsDoubleStatusSortingPagingSource(
    private val service: EventsApiService,
    private val status1: Int,
    private val status2: Int,
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, ResultModel> {
        ...
        return try {
            val response =
                service.getEventsDoubleStatusSorting(status1, status2, sorting, page)
    ...
    class Factory {
        fun create(
            service: EventsApiService, status1: Int, status2: Int,
            sorting: String
        ) =
            EventsDoubleStatusSortingPagingSource(service, status1, status2, sorting)
    }
}
