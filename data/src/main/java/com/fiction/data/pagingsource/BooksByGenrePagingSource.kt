...
class BooksByGenrePagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, BookItemResponse> {
        ...
        if (NetworkStatus.isNetworkConnected) {
            return when (val data = makeApiCall {
                analyzeResponse(service.getGenreById(genreId = genreId, page = page, limit = pageSize))
            }) {
    ...
    class Factory {
        fun create(
            ...
            BooksByGenrePagingSource(service, genreId, imagesDao, pageSize)
    }
}